...
- 第 121 轮精选冻结判词与判词结果（同题同判卷器，见 `evidence/round121/`）；
- 第 122 轮精选冻结判词、判词结果与脱敏逐棒账（同题同判卷器，见 `evidence/round122/`；逐棒账只含脱敏账字段，不含说明文本、代码、模型输出与 sha，第三方可重算 R／Z）；
- 第 118／119 轮精选冻结判词与判词结果（Z 轴打平、预检闸毙注，见 `evidence/round118/`、`evidence/round119/`）。

## Excluded

- API 凭证、`.env`、本机绝对路径；
- 模型原始 transcript、reasoning、私人会话与行为画像；
- 锁文件、运行日志（全量原始账）、缓存与逐轮代码库——第 122 轮起随轮导出**脱敏逐棒账**（`evidence/round122/per-round-account.csv`），全量原始日志仍在边界外；
- 第 123 轮及以后尚未收口的在途结果；
- 来源或再发布权未逐项确认的外部材料。
