...
- **已实现**：零网络词面基线；`select / compact / recall`；可恢复账本；审计记录；离线演示；单元测试；第 120／121 轮机械证据快照。
- **尚未实现**：AgentTeams 适配器、真实的 `investigator / fixer / verifier` 三 Agent 运行闭环、嵌入 provider、生产沙箱。
- **尚未证明**：上下文选择能提升任务成功率。第 120 轮正式结果是三种读法打平，选择臂的重复踩坑反而更多。第 121 轮在同一题上分出：死路记录写成禁令比写成中性描述重踩更少（R 0.55 对 0.65；Z 双 0.90 打平）——量的是记录写法的效应，不是任务成功率。
